...
from omr.bbox import BBox, find_lines, get_bbox, get_center
...
def staff_extract(
    ...
    for staffs in aligned_staffs.T:
        ...
        if not np.all(norm(unit_size) < unit_size_diff_th):          
            ...
            logger.info('StaffLine_extraction line 378')
            # continue
        valid_staffs.append(staffs)

    return np.array(valid_staffs).T, zones

def staff_extract_staffobj(
    staff_pred,
    splits: int = 8, 
    line_threshold: float = 0.8, 
    horizontal_diff_th: float = 0.1, 
    ...
    for rr in zones:
        print(rr[0], rr[-1], end=' ')
        rr = np.array(rr, dtype=np.int64)
        staffs = extract_part(staff_pred[:, rr], x_offset=rr[0], line_threshold=line_threshold, min_barheight=min_barheight)
        if staffs is not None:
            all_staffs.append(staffs)
            print(len(staffs), end= ' || ')
    print('')
    aligned_staffs: np.ndarray = align_staffs(all_staffs)
    ...
    return np.array(valid_staffs).T, zones
def extract_part(pred: ndarray, x_offset: int, line_threshold: float = 0.8, min_barheight = 8) -> List[Staff]:
    # Extract lines
    lines, _ = extract_line(pred, x_offset=x_offset, line_threshold=line_threshold, min_barheight=min_barheight)
...
def extract_line(pred: ndarray, x_offset: int, line_threshold: float = 0.8, min_barheight=8) -> Tuple[ndarray, ndarray]:
    ...
    centers, _ = find_peaks(norm, height=line_threshold, distance=min_barheight, prominence=1)
    centers -= 1
